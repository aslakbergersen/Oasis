...
def walls(x, on_boundary): 
    return on_boundary and x[2] < 0.320 - DOLFIN_EPS and x[2] > -0.18269 + DOLFIN_EPS

# inlet = 1
def inlet(x, on_boundary): return on_boundary and  x[2] < -0.18269 + DOLFIN_EPS
      
# outlet = 2
def outlet(x, on_boundary): return on_boundary and x[2] > 0.320 - DOLFIN_EPS


def create_bcs(V, sys_comp, **NS_namespce):
    Q =  5.21E-6 #6.77E-5   # From FDA
    r_0 = 0.006
    u_maks = Q / (4*r_0*r_0*(1-2/pi))  # Analytical, could be more exact numerical, different r_0
    inn = Expression('u_maks * cos(sqrt(pow(x[0],2) + pow(x[1],2))/r_0/2.*pi)', \
                       u_maks=u_maks, r_0=r_0)
...
def pre_solve_hook(velocity_degree, mesh, pressure_degree, V, **NS_namespace):
    ...
    Pv = FunctionSpace(mesh, 'CG', pressure_degree, constrained_domain=constrained_domain)
    ...
    for z_ in z:
        stats.append(StructuredGrid(V, N, [-0.006, -0.006, z_], vectors, dL, statistics=True))
    
    # Normals and domains to compute flux at at each point (z), inlet and outlet
    normal = FacetNormal(mesh)
    ...
    return dict(uv=Function(Vv), pv=Function(Pv), radius=radius, u_diff=Function(Vv),
                u_prev=Function(Vv), Vv=Vv, wall=wall, senterline=senterline, 
                domains=domains, Pv=Pv, z_senterline=z_senterline, normal=normal, 
                stats=stats)
    
def temporal_hook(tstep, info_red, steady, dt, radius, u_diff, pv, Pv, u_prev, u_, check_steady, 
                  domains, normal, Vv, uv, newfolder, mesh_resolution, mesh, p_, case,
                  z_senterline, wall, folder, senterline, stats, **NS_namespace):
    
    # steady state
    if tstep % check_steady == 0:
        #uv.assign(project(u_, Vv))
        #file = File(newfolder + "/VTK/nozzle_velocity_%0.2e_%0.2e_%d.pvd" % (dt, mesh.hmin(), tstep))
        #file << uv
        ...
        inlet_flux = assemble(dot(u_, normal)*ds(1), exterior_facet_domains=domains)
        outlet_flux = assemble(dot(u_, normal)*ds(2), exterior_facet_domains=domains)
        rel_err = (abs(inlet_flux) - abs(outlet_flux)) / abs(inlet_flux) #change with theoretical value
        ...
        if diff < 0.5: #2.5e-4:
            steady = True   # If the velocity does not change, save stats and kill the program
        else:
            u_prev.assign(uv)
    ...
    if senterline.number_of_evaluations == 100:
        file = File(newfolder + "/VTK/nozzle_velocity_%0.2e_%0.2e.pvd" % (dt, mesh.hmin()))
        file << uv
        file = File(newfolder + "/VTK/nozzle_pressure_%0.2e_%0.2e.pvd" % (dt, mesh.hmin()))
        file << pv
        
        # save the data from the centerline
        info = open(newfolder + "/Stats/nozzle_stats.txt", 'w')
        info = write_overhead(info, dt, tstep*dt, mesh.hmin(), case, mesh_resolution)
        ...
        for i in range(len(stats)):
            stats[i](uv)
            stats[i].toh5(0, tstep, filename=newfolder+"/VTK/slice_z_%s.h5" % (radius[i]))
