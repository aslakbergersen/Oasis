...
print dt
if h[-1] / h[-2] == 1:
    r_u0 = [log(E_u0[i-1][-1] / E_u0[i][-1]) / log(dt[i-1] / dt[i]) for i in range(1, len(E_u0))]
    r_u1 = [log(E_u0[i-1][-1] / E_u0[i][-1]) / log(dt[i-1] / dt[i]) for i in range(1, len(E_u1))]
    r_p =  [log(E_p[i-1][-1] / E_p[i][-1]) / log(dt[i-1] / dt[i]) for i in range(1, len(E_p))]
    k = dt
else:
...
for i in range(len(E_u0)):
    t = [dt[i]*5*k for k in range(len(E_u0[i]))][1:]
    semilogy(t, E_u0[i][1:])
    hold("on")
show()
...
for i in range(len(E_p)):
    t = [dt[i]*5*k for k in range(len(E_p[i]))][1:]
    semilogy(t, E_p[i][1:])
    hold("on")
show()
